<%@ Control Language="C#" AutoEventWireup="true" CodeFile="SmsCommunicationInjection.ascx.cs" CodeBehind="SmsCommunicationInjection.ascx.cs" Inherits="Arena.Custom.HDC.Twilio.SmsCommunicationInjection" %>
...
<asp:Literal ID="ltSelect" runat="server" />
            ...
            var tbEmail = $('input[id$="_tbEmail"]');
            var tbReplyTo = $('input[id$="_tbReplyTo"]');

            tbEmail.val('<%= FromEmail %>');
            tbReplyTo.val('<%= ReplyToEmail %>');
        }
    });
